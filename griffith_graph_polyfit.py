...
plt.scatter(np.log(plot_dias), np.log(avg_uts))
plt.show()

plt.scatter(dias, uts, s=50, alpha=0.7, label='Data')
# plt.plot(plot_dias, avg_uts, color='orange', linewidth=2.5, label='Trend')

coeffs = np.polyfit(np.log(plot_dias), np.log(avg_uts), 1)
x = np.linspace(np.log(min(plot_dias)), np.log(max(plot_dias)), 100)
...
plt.legend()
plt.tight_layout()
plt.savefig(fname='Graphs/uts_vs_dia_powerfit', dpi=600, bbox_inches='tight')
plt.show()
...
plt.legend()
plt.tight_layout()
plt.savefig('Graphs/uts_vs_area_powerfit', dpi=600, bbox_inches='tight')
plt.show()